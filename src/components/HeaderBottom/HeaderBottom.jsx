...
function HeaderBottom({ location }) {

    const pathName = location.pathname.startsWith('/courses') ? 'Courses' : location.pathname.startsWith('/blog') ? 'Blog':"";
    return (
...
</svg><span className="header__bottom--text pathText">{pathName}</span>
                </div>
            </div>
        </div>
    );
